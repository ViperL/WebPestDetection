...
class FRCNN(object):
    _defaults = {
        ...
        #--------------------------------------------------------------------------#
        "model_path"    : 'model_data/BestLossweight-139epoch.pth',
    ...
    def detect_image(self, image, crop = False):
        ...
        for i, c in list(enumerate(top_label)):
            ...
            clsResult += '类别:{};置信度:{:.2f};位置:[{},{},{},{}]\n'.format(predicted_class,score, top, left, bottom, right)
        ...
        return image,clsResult
    
    #---------------------------------------------------#
    #   获取中间特征层
    #---------------------------------------------------#   

    def detect_featuremaps(self, image, crop = False):
        image_shape = np.array(np.shape(image)[0:2])
        input_shape = get_new_img_size(image_shape[0], image_shape[1])
        image       = cvtColor(image)
        image_data  = resize_image(image, [input_shape[1], input_shape[0]])
